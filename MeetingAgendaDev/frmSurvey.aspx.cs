    ...
    public partial class frmSurvey : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    hdnAttendeesID.Value = System.Web.HttpUtility.UrlDecode(Request.QueryString["ID"].ToString());
                    //CGCipher.Decrypt(Request.QueryString["ID"].ToString(), "");

                    objclsSurvey = new clsSurvey();
                    dt = new DataTable();
                    objclsSurvey.AttendeesID = int.Parse(Request.QueryString["ID"]);
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (!IsSurveyFilled)
            {
                ...
                string SurveyBCCMailID = ConfigurationManager.AppSettings["survey.bcc.mail"].ToString();


                clsSendMail objclsSendMail = new clsSendMail();
                objclsSendMail.SendMail(dt.Rows[0]["AEMailID"].ToString().Trim(), "", SurveyBCCMailID, "Client Review Meeting Response", MailBody(dt.Rows[0]["AEName"].ToString().Trim(), dt.Rows[0]["AttendeeName"].ToString().Trim(), dt.Rows[0]["ClientName"].ToString().Trim(), dt.Rows[0]["ClientNo"].ToString().Trim()), "");
