    ...
    if (strMoney != "") {
        ...
        if (!strMoney.match(/^\d+$/)) {
            ...
            for (var i = MoneyLength; i > 0; i = i - 3) {
                strOutput = strMoney.substring(i - 3, i) + "," + strOutput;
            }
            inputText.value = "$ " + strOutput.substring(0, strOutput.length - 1);
            return true;
        }
    }
    return true;
...
function mngMoney(inputText) {
    ...
    if (strMoneyZero != "") {
        inputText.value = strMoney.substring(2, strMoney.length).replace(/,/g, "");
    }
}
